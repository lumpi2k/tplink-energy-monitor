...
router.ws('/', function (ws, req) {
  ws.on('message', (msg) => {
    ...
    if (message.requestType === 'getCachedData') {
      ...
      ws.send(
        dataBroadcaster.generatePayload(
          'realtimeUsage',
          deviceId,
          cachedData.realtimeUsage,
        ),
      );
      ws.send(
        dataBroadcaster.generatePayload(
      ...
      ws.send(
        dataBroadcaster.generatePayload(
          'powerState',
          deviceId,
          cachedData.powerState,
        ),
      );
      dataLogger.getLogEntriesForDevice(deviceId, (loggedData) => {
        ws.send(
          dataBroadcaster.generatePayload('loggedData', deviceId, loggedData),
        );
      });
      ...
      if (device !== undefined) {
        device.togglePowerState().then((result) => {
          ws.send(
            dataBroadcaster.generatePayload('powerState', deviceId, {
              isOn: result,
              uptime: 0,
            }),
          );
